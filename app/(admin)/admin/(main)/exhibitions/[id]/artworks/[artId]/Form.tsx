...
import { getExhibitionArtwork, updateExhibitionArtWork } from "@/actions";
...
const UploadExhibitionArt: React.FC<{
  categories: Category[];
  data: Awaited<ReturnType<typeof getExhibitionArtwork>>;
}> = ({
  categories,
  data: {
    art_image,
  ...
  const [url, setUrl] = useState(art_image);
      ...
      <Formik
        ...
        onSubmit={(data, { resetForm, setSubmitting }) => {
          (async () => {
            try {
              let image = null;
              if (data.art_image !== art_image) {
                image = await uploadToCloudinary(data.art_image);
              }
              await updateExhibitionArtWork(
                {
                  ...data,
                  art_image: image ? image : art_image,
                  exhibition_id,
                },
                id
              );
              resetForm();
              setSubmitting(false);
              toast.success("Exhibition Art updated Successfully");
