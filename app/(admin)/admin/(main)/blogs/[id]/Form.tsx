...
import { addBlog, addCollectible, updateBlog } from "@/actions";
...
const FormComp = ({ blog }: { blog: Blog }) => {
  const { cover_image, content, title } = blog;
          ...
          (async () => {
            try {
              ...
              if (data.cover_image !== cover_image) {
                image = await uploadToCloudinary(data.cover_image);
              }
              await updateBlog(blog.id, {
                ...data,
                cover_image: image ? image : cover_image,
              });
              resetForm();
              setSubmitting(false);
