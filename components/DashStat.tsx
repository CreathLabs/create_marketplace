...
import { useAuthCore, useEthereum, useConnect } from '@particle-network/authkit';
...
const getAccountBalanceInUSD = async (address: string, ethersProvider: ethers.providers.Web3Provider) => {
...
const DashStat = ({ title, value }: { title: string; value: string }) => {
    ...
    const initializeProvider = async (provider: any) => {
      ...
      // Get balance in USD
      const usdBalance = await getAccountBalanceInUSD(accounts[0], ethersProvider);
    ...
    return (
      <div className="flex justify-between items-center ">
        <div className="space-y-4">
          ...
          <h1 className="font-Playfair text-[26px] font-bold ">{balance ? `$ ${balance}` : `ETH ${ethBalance}`}</h1>
        </div>
